<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="兼职编号" prop="jianzhibianhao">
            <el-input v-model="ruleForm.jianzhibianhao" 
                placeholder="兼职编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jianzhibianhao" label="兼职编号" prop="jianzhibianhao">
              <el-input v-model="ruleForm.jianzhibianhao" 
                placeholder="兼职编号" readonly></el-input>
          </el-form-item>
        </div>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='jianzhibianhao'){
            this.ruleForm.jianzhibianhao = obj[o];
	    this.ro.jianzhibianhao = true;
            continue;
          }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaoyuanjianzhi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaoyuanjianzhiCrossAddOrUpdateFlag = false;
						 this.parent.search();
